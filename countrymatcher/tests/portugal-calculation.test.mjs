...
test('D8 is unsuitable below the mandatory threshold without a plan to increase income', () => {
  ...
  assert.ok(d8.blockers.some((message) => message.includes('ниже обязательного порога')));
  assert.deepEqual(d8.actions, []);
});
...
test('D8 rejects an incompatible passive-income basis', () => {
  ...
  assert.ok(d8.blockers.length > 0);
});
...
test('D7 does not treat active foreign remote salary as own passive income', () => {
  ...
  assert.deepEqual(d7.actions, []);
});

test('D2 remains conditional because the questionnaire does not verify the Portuguese business basis', () => {
...
test('D2 turns creation of a real independent basis into a substantive condition', () => {
  ...
  assert.equal(d2.routeStatus, 'SUITABLE_WITH_CONDITIONS');
  assert.equal(d2.basisMissing, true);
  assert.ok(d2.conditions.some((condition) => condition.includes('предпринимательский проект')));
});

test('D1 is conditional because the questionnaire does not verify a Portuguese employment contract', () => {
  ...
  assert.equal(confirmed.routeStatus, 'SUITABLE_WITH_CONDITIONS');
  assert.equal(willing.routeStatus, 'SUITABLE_WITH_CONDITIONS');
  assert.ok(willing.conditions.some((condition) => condition.includes('работодателя в Португалии')));
  assert.equal(declined.routeStatus, 'SUITABLE_WITH_CONDITIONS');
});
...
test('partner and child change the D7 and D2 family thresholds from the package formula', () => {
  ...
  assert.equal(route(contractor, 'PT_D2_INDEPENDENT').routeStatus, 'SUITABLE_WITH_CONDITIONS');
});
...
test('Portugal result uses researched cities, family, long-term, work, schools, pets, LGBT, sources, and route-scoped reviews', () => {
  ...
  assert.match(result.petSummary, /опасных и потенциально опасных собак/i);
...
test('public matcher loads Portugal without adding Portugal-specific questionnaire fields', async () => {
  ...
  assert.match(app, /portugal-adapter\.js\?v=7\.1\.0/);
  assert.match(app, /portugal-research-v3\.0\.json\?v=7\.1\.0/);
...
test('Portugal corrective actions omit paired conditions and requirements that say the same thing', () => {
  ...
  assert.deepEqual(uniqueRouteActions(route(result, 'PT_D2_INDEPENDENT')), [
    'Оформить договор независимых услуг либо подготовить жизнеспособный предпринимательский проект и зарегистрировать деятельность.',
  ]);
...
test('Portugal cities expose the expected size-first matcher categories', async () => {
  ...
  assert.deepEqual(categories.Лиссабон, ['Большой город', 'Столица', 'Самый дорогой']);
});
