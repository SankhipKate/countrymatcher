...
test('active packages mark exactly the approved 33 separate-basis requirements', () => {
  const expected = [
    ...
    'PT_EMPLOYMENT_BASIS', 'PT_INDEPENDENT_BASIS', 'PT_ENTREPRENEUR_BASIS', 'PT_STARTUP_BASIS',
    'PT_HQ_NATIONAL_BASIS', 'PT_BLUE_CARD_BASIS', 'PT_RESEARCHER_BASIS', 'PT_STUDY_ADMISSION',
    'PT_ICT_BASIS', 'PT_FAMILY_SPONSOR_BASIS', 'PT_EU_FAMILY_BASIS', 'PT_ARI_INVESTMENT_BASIS',
    'PT_SKILLED_JOB_SEEKER_BASIS',
  ].sort();
  const actual = [spain, argentina, uruguay, portugal].flatMap((pkg) => pkg.routes.flatMap(({ requirements = [] }) =>
  ...
  for (const requirementId of [
    ...
    'PT_PROTECTION_BASIS', 'PT_HUMANITARIAN_BASIS',
  ]) assert.equal(requirementById(requirementId.startsWith('ES_') ? spain : requirementId.startsWith('AR_') ? argentina : requirementId.startsWith('UY_') ? uruguay : portugal, requirementId).requires_separate_basis, undefined, requirementId);
});
...
test('pure FAMILY and pure ICT routes follow one active-package visibility policy', () => {
  for (const pkg of [spain, argentina, uruguay, portugal]) {
    for (const route of pkg.routes) {
      if (
        route.route_type === 'FAMILY' ||
        route.route_type === 'INTRA_COMPANY_TRANSFER'
      ) {
        assert.equal(
          route.publishable,
          false,
          `${pkg.country_id}/${route.route_id}`,
        );
      }
    }
  }

  for (const [pkg, routeId] of [
    [argentina, 'AR_SPECIALIST_TRANSFER'],
    [uruguay, 'UY_TEMP_SPECIALIST'],
  ]) {
    const route = pkg.routes.find(
    ...
    assert.equal(
      route.covers_categories.includes('INTRA_COMPANY_TRANSFER'),
      true,
    );
