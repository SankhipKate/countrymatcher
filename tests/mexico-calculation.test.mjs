...
test('public matcher loads Mexico, its adapter, its flag and researched cities', async () => {
  ...
  assert.match(app, /mexico-adapter\.js\?v=6\.0\.0/);
  assert.match(app, /mexico-research-v3\.0\.json\?v=6\.0\.0/);
  assert.match(app, /countryId === 'MX' \? '🇲🇽'/);
  assert.match(app, /\['AR', 'PY', 'PT', 'MX'\]\.includes\(countryId\)/);
  assert.match(fx, /quotes=EUR,ARS,MXN/);
  assert.match(fx, /\['EUR', 'ARS', 'MXN'\]/);
});
